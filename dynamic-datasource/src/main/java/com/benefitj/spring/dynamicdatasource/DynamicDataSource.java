...
public class DynamicDataSource extends AbstractDataSource implements InitializingBean {
    ...
    if (dataSource == null) {
      if (lookupKey != null) {
        synchronized (resolvedDataSources) {
          // 创建数据源连接
          if ((dataSource = resolvedDataSources.get(lookupKey)) == null) {
            try {
              dataSource = getDataSourceFactory().create(lookupKey);
              this.resolvedDataSources.put(lookupKey, dataSource);
            } catch (Exception e) {
            }
          }
        }
      } else if ((this.lenientFallback)) {
  ...
  protected Object determineCurrentLookupKey() {
    Object lookupKey = getLookupKeyContext().get();
    return lookupKey != null ? lookupKey : this.defaultTargetDataSource;
  }
